#####################################
## StockBot - Fast Trader Version 4.0
#####################################
...
true.positions <- function(vector){ # this function returns the positions that are true in a vector
  ...
  return(positions_true)
  
} # end of true.positions
...
key_big_data_api <- "giroud" # this will be the variable with the key for the big data for all
...
#reset_tickers <- c("acb", "el", "mik", "msft", "tsla") # this are the tickers that will start on tuesday 02/16 so we have to sell them first

#reset_shares <- c(850, 43, 707, 45, 13)
...
money_account <- 60000/8 # We are dividing the money equally into 8 accounts
...
###################################################
## StockBot - Fast Trader Version 4.0 - SOURCE CODE
    ...
    if(is_weekday == TRUE && is_holiday == FALSE && is_market_open == TRUE){
      ...
      repeat{ # daily repeat for trade hours
        ...
        if(wrap_up_time == TRUE){ # We will wrap up, meaning, sell stocks and then buy the bank stock
          ...
          url <- paste("https://api.tiingo.com/iex/",ticker_bank,"/prices?token=9771c442e25ca9dad5743e1cde3891b77bc9bad3","&startDate=", today_date, "&resampleFreq=1min&format=csv", sep = "" )
          ...
          quantity_to_buy_bank <- transaction$quatity
          ...
          if(times_the_bot_runned == 0){
          ...
          } else if(times_the_bot_runned < 2){      # this closes the condiditonal if times_the_bot_runned == 0  and opens the new statemtn for the conditional  
            ...
            differences <- c() # creating a tempporary vector to store the normalized
            ...
            for(i in 1:length(vector_tickers)){
              ...
              base_data[[i]] <- (rbind(old_data, new_data))[2:3, ] 
              
              differences[i] <- current_price[i] - ((base_data[[i]])[1, "close"])    
            ...
            if(length(non.na.positions.locator(bougth_tickers)) == 0){                    # starting the conditionals to sell if is zero it means we are completely out of the market
              ...
              positions_to_buy <- two.maximun.positions(differences)
              
              if(differences[positions_to_buy[1]] >= 0){
                ...
                if(vector.comparison(receipt,error) == TRUE){
                  ...
                  if(differences[positions_to_buy[2]] >= 0){
                  ...
                  if(selling_opportunity[position_to_sell[2]] <= 0 && is.na(selling_opportunity[position_to_sell[2]]) == FALSE){  
                    ...
                    if(vector.comparison(receipt, error) == TRUE){
                      ...
                      if(selling_opportunity[position_to_sell[1]] > 0 && is.na(selling_opportunity[position_to_sell[1]]) == FALSE) { # check this conditional  selling_opportunity[position_to_sell[2]] < 0  if we couldnt buy x we sell y
                        ...
                        if(differences[positions_to_buy[1]] >= 0 && is.na(bougth_tickers[positions_to_buy[1]]) == TRUE){
                          ...
                          if(vector.comparison(receipt,error) == TRUE){
                            ...
                            if(selling_opportunity[position_to_sell[2]] > 0 && is.na(selling_opportunity[position_to_sell[2]]) == FALSE){  # check this conditional  selling_opportunity[position_to_sell[2]] < 0  if we couldnt buy x we sell y
                              ...
                              if(differences[positions_to_buy[2]] >= 0 && is.na(bougth_tickers[positions_to_buy[2]]) == TRUE){
...
## StockBot - Fast Trader Version 3.0 - END OF THE SOURCE CODE
#################################################################
